...
class MinEuclideanDistBlockACS(nn.Module):
    ...
    def forward(self,x_t,previous_distance):
        """
        ...
        x_t = x_t.unfold(2, self.shapelets_size, 1).contiguous()
        x_t = torch.cdist(x_t, self.shapelets, p=2)
        ...
        x_t, _ = torch.min(x_t, 2)
        x_t,__=torch.min(x_t,1)
        x_t=x_t.reshape(x_t.shape[0],1,x_t.shape[1])
        if previous_distance=='transform':
            finally_distance=x_t
        else:
            finally_distance=torch.where(x_t<previous_distance,x_t,previous_distance)
        return finally_distance
