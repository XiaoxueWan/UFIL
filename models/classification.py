...
class LearningShapelets(nn.Module):
    ...
    def online_monitor(self, Prototypes):
        ...
        for t in range(self.shapelet_length,4000+1,1):    #代表遍历的数据点
            ...
            num+=1
        #print(result_list)
        plot_sub_fig(X,result_list)
        plot_sub_fig(X,Y_max_list)
        plot_sub_fig(X,Score_list)
        plot_sub_fig(X,ENTROPY_list)
        return
